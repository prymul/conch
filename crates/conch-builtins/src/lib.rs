...
impl Builtin for Exit {
    fn run(&self, _shell: &mut Shell, args: &[String]) -> i32 {
        let code = args
            .first()
...
impl Builtin for Pwd {
    fn run(&self, shell: &mut Shell, _args: &[String]) -> i32 {
...
mod tests {
    ...
    fn export_with_value_sets_env_var() {
        let mut shell = Shell::new();
        Export.run(&mut shell, &["FOO=bar".to_string()]);
    ...
    fn bare_export_promotes_shell_var() {
        ...
        Export.run(&mut shell, &["FOO".to_string()]);
        assert_eq!(shell.get_var("FOO"), Some("bar"));
    ...
    fn cd_updates_cwd() {
        ...
        let status = Cd.run(&mut shell, &["/tmp".to_string()]);
        assert_eq!(status, 0);
    ...
    fn cd_nonexistent_dir_fails() {
        let mut shell = Shell::new();
        let status = Cd.run(&mut shell, &["/no/such/path/hopefully".to_string()]);
        assert_eq!(status, 1);
    }
}
